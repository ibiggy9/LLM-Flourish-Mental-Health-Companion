...
import {View, Text, useWindowDimensions, TextInput, ScrollView, KeyboardAvoidingView, Keyboard, Image, Platform, TouchableOpacity, TouchableWithoutFeedback } from 'react-native'
...
export default function RefreshLogin({navigation, route}) {
    if(route.params){
  ...
  return (
    <TouchableWithoutFeedback style={[{width: width, height:height}, tw`bg-indigo-900`]}> 
        <KeyboardAvoidingView style={tw`flex-1 bg-indigo-900`} behavior={'padding'}>
        ...
        <>
            <View style={tw` mb-1 items-center flex-row`}>
                ...
                <Text style={tw` ml-3 text-white text-2xl font-bold text-center`}>Login To Confirm Account Deletion</Text>
                </MotiView>
                ...
                <MotiView from={{scale:0, opacity:0}} animate={{scale:1, opacity:1}} transition={{type:'timing', duration:1000}} style={tw`mt-4 flex-col items-center `}>
                  {!showCheck &&
                    ...
                    <TouchableOpacity onPress={async () => {
                        ...
                        onGooglePress().then(()=> route.params ? navigation.navigate('ConfirmDelete') : null)
                    ...
                    try {
                        ...
                        requestedScopes: [
                            ...
                            if(!userToken.email){
                                ...
                                signInWithEmailAndPassword(auth, decoded.email, userToken.user).then(()=> navigation.navigate('ConfirmDelete', {refreshed:true}))
